...
from cassle.utils.dali_dataloader import (
    ...
    def train_dataloader(self) -> DALIGenericIterator:
        ...
        if self.multicrop:
            ...
            train_pipeline = PretrainPipeline(
                data_dir / train_dir if train_dir is not None else data_dir,
                batch_size=self.batch_size,
                transform=transform,
                device=dali_device,
                device_id=device_id,
        ...
        if self.online_eval:
            loaders.update(online_eval=prepare_data(
                dataset,
                data_dir=data_dir,
                train_dir=train_dir,
                val_dir=self.extra_args["val_dir"],
                batch_size=self.online_eval_batch_size,
